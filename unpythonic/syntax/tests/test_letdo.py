...
def runtests():
    ...
    with testset("haskelly syntax"):
        ...
        # With more than one binding the delimiters surrounding the bindings subform
        # are required, even in this format where they are somewhat redundant.
        result = let[[x << 1, y << 2] in x + y]  # noqa: F821
        test[result == 3]

    # single binding special syntax, no need for outer delimiters
    with testset("special syntax for single binding case (classic format)"):
        result = let[x, 1][2 * x]
        test[result == 2]
    ...
    with testset("special syntax for single binding case (modern format)"):
        result = let[x << 1][2 * x]
        test[result == 2]
        result = let[[x << 1] in 2 * x]
        test[result == 2]
