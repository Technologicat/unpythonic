...
- `unpythonic.net.client`: `import readline` moved from module top into `connect()`, and wrapped in the same three-tier fallback (`readline` → `pyreadline3` → `None` with graceful degradation) used by `mcpyrate.repl.macropython`. POSIX behaviour is unchanged; the module is now importable on Windows, which unblocks docs generation, test collection, and the eventual Windows port of the REPL subsystem.
- `unpythonic.net.tests.fixtures.nettest`: reworked to bind on port 0 (kernel-assigned), removed the `sleep(0.05)` race-condition bandage (the listening socket now exists before either worker thread starts, so the TCP stack itself is the synchronization primitive), and re-raises worker-thread exceptions in the main thread instead of swallowing them into a `print(err)`. Makes the existing `test_msg.py` / `test_util.py` tests robust against port collisions and CI load.
