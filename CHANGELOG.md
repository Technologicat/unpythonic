...
- Tag continuation closures (generated by the `with continuations`), for introspection. A continuation closure now has the attribute `is_continuation` (and redundantly, it is set to `True`).
