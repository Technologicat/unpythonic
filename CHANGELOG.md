**0.14.2** (in progress; updated 7 August 2020) - *"Greenspun" [edition](https://en.wikipedia.org/wiki/Greenspun%27s_tenth_rule)*:
...
- **Multiple dispatch**. The `generic` decorator makes a generic function with multiple dispatch. Arity and type annotations determine which method of the generic function a specific call of the function is dispatched to.
  ...
  - Based on my own initial experiments with this feature, the machinery itself works well enough, but to really shine - just like resumable exceptions - multiple dispatch needs to be used everywhere, throughout the language's ecosystem. It exhibits a different philosophy, which is in practice impossible to tack on. To become widespread, it must be a feature of the language right from the start.
