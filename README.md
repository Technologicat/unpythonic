...
Hence to use a callable as a value, it always needs the ``lambda e: ...``, whether or not it actually uses the environment. Otherwise the callable itself will be called with the environment as its argument (likely not what was intended). Examples of correct usage:
