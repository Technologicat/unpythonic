...
To be useful, a Python-based language extension doesn't need to make it into
the Python core, or even be desirable for inclusion into the Python core.
Conciseness and abstractions matter; language customization helps when
functions and even syntactic macros leave code repetitive or unreadable.
...
a custom language that compiles to Python AST or bytecode. Pydialect is geared
toward creating languages that extend Python and look mostly like Python, but
extend or modify its syntax and/or semantics. Hence dialects.
...
its code was not generated by Pydialect.

The lang-import is a customization added by the Pydialect. This ensures that
the module will immediately fail if run under standard Python, because there
...
top-level module (no dots in the name). Strictly, the dialect finder wouldn't
need to care about this, but IDEs and tools in general are much happier with
code that does not contain syntax errors. (This allows using standard tools
with dialects that do not introduce any new surface syntax.)
...
(the missing Python AST docs) is a highly useful resource here.
...
``ast_literal[tree]``; this may not always be what you want, if you're aiming
to splice in a block of statements, but it's unavoidable given Python's AST
representation.
